...
Adafruit_NeoPixel statusPixel(kStatusLedCount, kStatusLedPin, NEO_GRB + NEO_KHZ800);
...
static void updateStatusFromResult(int topIndex) {
  // Map the most confident class to a color: Glove -> red, Null -> blue.
  if (topIndex == 0) {
...
static int topPredictionIndex() {
  int bestIndex = 0;
  float bestScore = tf.output(0);
  for (int i = 1; i < kCategoryCount; ++i) {
    float score = tf.output(i);
    if (score > bestScore) {
      bestIndex = i;
      bestScore = score;
    }
...
void setup() {
  Serial.begin(115200);
  Serial.println("Starting Example-LED Indicator...");
...
void loop() {
  ...
  int topIndex = topPredictionIndex();
  updateStatusFromResult(topIndex);
  Serial.printf("Indicator -> %s\n", kCategoryLabels[topIndex]);
